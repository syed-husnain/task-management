...
<head>
    ...
    <title>Carton Data Authorizer Panel</title>
...
<body>
        ...
        <nav class="navbar navbar-default navbar-static-top" role="navigation" style="margin-bottom: 0">
            <div class="navbar-header"> <button type="button" class="navbar-toggle" data-toggle="collapse"
                ...
                <img src="{{ asset('resources/assets/images/logo.png') }}" width="130px;"
                    ...
                    href="{{ route('daDashboard') }}">Carton Data Authorizer Panel</a> </div>
            <!-- /.navbar-header -->
            <ul class="nav navbar-top-links navbar-right" style="margin: 0">
                ...
                <li class="nav-item dropdown"> <a id="navbarDropdown" class="nav-link dropdown-toggle" href="#"
                        role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>Welcome {{ Auth('dataAuthorizer')->user()->name }}
            ...
            </ul> <!-- /.navbar-top-links --> @include('layouts.da-sidebar')
        </nav>
        <div id="page-wrapper">
            <div class="pt-10"> @include ('layouts.partials._notifications') @yield('content') </div>
        </div>
    </div> {{-- bower components --}} <script type="text/javascript"
    ...
    <script>
        var routes = {
            changeDAproductStatus: "{{ route('changeDAproductStatus') }}",
            changeDArequestedProductApproval: "{{ route('changeDArequestedProductApproval') }}",
            changeDArequestedProductStatus: "{{ route('changeDArequestedProductStatus') }}",
            changeDAproductStatus1: "{{ route('changeDAproductStatus1') }}",
        };
